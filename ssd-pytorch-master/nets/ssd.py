...
class SSD300(nn.Module):
    def __init__(self, num_classes, backbone_name, pretrained = False):
        ...
        if backbone_name    == "vgg":
            ...
            mbox            = [4, 6, 6, 6, 4, 4]  # 即num_anchors
            
            loc_layers      = []
            conf_layers     = []
            backbone_source = [21, -2] # 21层是conv层有输出通道，而
            #---------------------------------------------------#
            #   在add_vgg获得的特征层里
            ...
            #-------------------------------------------------------------#  
            for k, v in enumerate(self.extras[1::2], 2):  # 每隔两个卷积取出一个卷积层，self.extras 是一个列表，[1::2] 表示从索引为 1 的位置开始，以步长 2 进行切片。最后的参数 2 是 enumerate 函数的起始索引，表示从索引为 2 开始计数。
                loc_layers  += [nn.Conv2d(v.out_channels, mbox[k] * 4, kernel_size = 3, padding = 1)]  # 回归头
                conf_layers += [nn.Conv2d(v.out_channels, mbox[k] * num_classes, kernel_size = 3, padding = 1)]  # 分类头
        else:
    ...
    def forward(self, x):
        ...
        if self.backbone_name == "vgg":
            for k in range(23):  # conv4-3，即38*38*512
                x = self.vgg[k](x)
        else:
            for k in range(14):
        ...
        for (x, l, c) in zip(sources, self.loc, self.conf):
            loc.append(l(x).permute(0, 2, 3, 1).contiguous())  # 回归
            conf.append(c(x).permute(0, 2, 3, 1).contiguous())  # 分类
