...
if __name__ == '__main__':
    ...
    class AnchorBox_for_Vision():
        def __init__(self, input_shape, min_size, max_size=None, aspect_ratios=None, flip=True):
            ...
            self.aspect_ratios = []
            for ar in aspect_ratios: # 比如aspect_ratios[0]=[1,2],输入1，输出两个值1, 1;输入2，输出两个值2, 1/2
                self.aspect_ratios.append(ar)
                self.aspect_ratios.append(1.0 / ar)
        ...
        def call(self, layer_shape, mask=None):
            ...
            for ar in self.aspect_ratios:
                # 首先添加一个较小的正方形
                if ar == 1 and len(box_widths) == 0:
                    box_widths.append(self.min_size)  # 先验框的短边
                    box_heights.append(self.min_size)
                # 然后添加一个较大的正方形
                elif ar == 1 and len(box_widths) > 0:
                    box_widths.append(np.sqrt(self.min_size * self.max_size))  # 先验框的长边
                    box_heights.append(np.sqrt(self.min_size * self.max_size))
                # 然后添加长方形
                elif ar != 1:
                    box_widths.append(self.min_size * np.sqrt(ar))
            ...
            # --------------------------------- #
            #   每一个特征层对应的步长
            #   3x3的特征图的一个像元对应原影像中(300*300)中是100*100的大小，即300*300影像步长为100
            # --------------------------------- #
            step_x = img_width / layer_width
            step_y = img_height / layer_height
    ...
    feature_heights, feature_widths = get_vgg_output_length(input_shape[0], input_shape[1])
    # 对先验框的数量进行一个指定 4,6,6,6,4,4
    aspect_ratios  = [[1, 2], [1, 2, 3], [1, 2, 3], [1, 2, 3], [1, 2], [1, 2]]

    anchors = []
    for i in range(len(feature_heights)): # 调用类进行先验框的计算
